...
module.exports.sampleDataFromAttrs = function sampleDataFromAttrs(attrs) {
  ...
  for(var i = 0; i < attrs.length; i++) {
    var attr = attrs[i];
    sampleData.push(stringUtils.camelize(attr.name) + ': ' + attr.sampleValue);
  }
...
module.exports.entityAttrs = function entityAttrs(entityOptions) {
  ...
  for(var name in entityOptions) {
    ...
    var label = inflection.humanize(name).replace(/[_-]/, ' ');
    attrs.push({
      name: attrName,
      camelizedName: stringUtils.camelize(attrName),
      label: label,
      sampleValue: sampleValue(dasherizedType),
    });
  }
...
module.exports.buildNaming = function buildNaming(entityName) {
    ...
    var underscoredModuleName = stringUtils.underscore(entityName);
    ...
    return {
      ...
      underscoredModuleName: underscoredModuleName,
      underscoredModuleNamePlural: inflection.pluralize(underscoredModuleName),
    };
};
